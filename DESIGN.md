...
The homepage runner band sits inside the hero itself, between the call-to-action block and the 01/02/03 capability row, and it is the one inverted surface in the hero: an ink panel with paper drawn on it, which is what separates the game from the warm paper above and below it. The player is a small paper bird whose wing beats as it runs and stays raised in the air. Obstacles are colour-coded by the move they demand: paper for the ones a single jump clears, orange for the tall barrier that needs a double jump and for the ceiling bar that can only be ducked under — orange means "this one needs something other than a jump", alongside its other uses on the crashed bird and a new best score. Behind them, one of six tinted scenes — city, desert, forest, sea, mountains, night — scrolls at a third of the world speed, swapping every ten seconds with a cross-fade. The tint is the only colour in the panel besides the accent, and it carries the world's identity. Each world is titled, centred at the top of the panel, with one real client, project or backer — that card is the point of the game, which exists to walk a visitor through the roster one world at a time. It is staged as a piece of the brand rather than a HUD readout: a filled chip in the world's tint carrying the kind, a counter beside it saying how many worlds there are to find, the name set big in the site's display voice, and the sector or category beneath. It rises into place as it fades in, and obstacles pass in front of it, which keeps it scenery rather than interface. The roster is drawn from the case-study and project catalogues, so it grows with the site rather than with the game, and the scenes themselves are never named on screen — the visitor is meant to notice the client, not the hills. The panel never animates on load — the run starts only on a deliberate key press or tap and freezes when scrolled out of view — and the world keeps one logical size everywhere, rendered smaller below 640px so a phone sees more of the track instead of losing a third of its screen.
