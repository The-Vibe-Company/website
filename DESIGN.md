...
The homepage runner band sits inside the hero itself, between the call-to-action block and the 01/02/03 capability row, and it is the one inverted surface in the hero: an ink panel with paper drawn on it, which is what separates the game from the warm paper above and below it. The player is a small paper bird whose wing beats as it runs and stays raised in the air. Obstacles state the move, not the object: paper to jump, orange to double jump, yellow to duck. Shape says what a thing is, colour says what to do about it. Slabs are slim, round their corners only on the edges facing the bird so each points the way it hangs, and wear black diagonal hazard stripes when they need more than a plain jump. A ceiling bar's length is measured in how long it keeps you down — a quarter second to nearly a full one — so it stays a long duck at speed rather than flicking past; only its lower lip is solid — the same 12px the standing slabs are wide, so every obstacle is the same bar just hung differently — with the rest hanging as threads from the roof; flying predators are drawn as creatures with a beating wing that sweeps outside their hit box, which only ever works in the player's favour — orange means "this one needs something other than a jump", alongside its other uses on the crashed bird and a new best score. Behind them, one of six tinted scenes — city, desert, forest, sea, mountains, night — scrolls at a third of the world speed, swapping every ten seconds with a cross-fade. The tint is the only colour in the panel besides the accent, and it carries the world's identity. Each world belongs to one real client, project or backer, and their name is set as a giant hollow word across the sky — outlined rather than filled, in the world's tint, so obstacles and decor stay readable straight through it, and sized to the panel so a long name shrinks instead of running off a phone. A single letterspaced mono kicker sits above it — the kind and the sector, nothing else. The name is an announcement rather than a permanent label: it rises in, holds about four seconds, then drifts up and out, leaving the sky clear for the rest of the world's ten seconds. This is the reason the game is on the page at all: it tours the roster one world at a time, in a random order on each visit, drawn from the case-study and project catalogues so it grows with the site rather than with the game. The scenes are never named on screen — the visitor is meant to notice the client, not the hills. Whenever a run is not in progress — before the first press as much as after a loss — a near-solid ink veil drops over the whole world, leaving only the score line above the canvas and the replay message on top of it to read. Sound follows the same rule as the veil: it exists only while a run does. The bed is a pad of five voices, each breathing on its own slow cycle, cutting between Am, Dm, Gm and F every 2.4 seconds. The change is a cut, not a glide: every voice jumps straight to its new note, which is what gives the progression its edge, while the level stays continuous so an abrupt change never clicks. Over it sits a melody of a handful of chord tones with room to breathe — never a tune to follow, just enough to be recognised. Three sections take turns, eight bars each: A states the progression plainly with one ringing note per chord, B colours the same roots with sevenths and opens the line out, C keeps the harmony and chops it, pulsing the pad in a staccato pattern. They all share the same roots and the same cut, which is what lets one follow another without a seam. The filter opens as the run speeds up: brighter, never louder. Jumps answer with a soft filtered breath upward, the second one higher; ducking is a short swish passing overhead; a crash is a dull thud under muffled noise. Every voice goes through a low-pass, which is what keeps the whole thing from turning metallic. It is all synthesised — no audio files ship with the page — and no audio context exists at all until someone deliberately starts a run. A speaker toggle sits at the end of the score line and the choice is remembered. The panel never animates on load — the run starts only on a deliberate key press or click and freezes when scrolled out of view — and it does not exist below the desktop breakpoint at all: the game needs a keyboard, and two thumb buttons were never going to carry a double jump and a duck, so on a phone the hero runs straight into the capability row.
