...
The homepage runner band sits inside the hero itself, between the call-to-action block and the 01/02/03 capability row, and it is the one inverted surface in the hero: an ink panel with paper drawn on it, which is what separates the game from the warm paper above and below it. Inside, everything is reduced to essentials — a mono readout row (controls left, score and best right), a single hairline ground, paper slabs for obstacles, and orange kept for the flying slab, the crashed player and a new best score. The player is the company mark: a paper tile carrying the logo's double chevron, whose outer arms lift and drop like wings as it runs and stay spread while it is airborne. The panel never animates on load — the run starts only on a deliberate key press or tap and freezes when scrolled out of view — and the world keeps one logical size everywhere, rendered smaller below 640px so a phone sees more of the track instead of losing a third of its screen.
