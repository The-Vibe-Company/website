...
An **experimental homepage preview** lives at `/v2` — the existing warm-paper homepage (same `TopNav`, `Services`/`Process`/`Proof`/`FinalCTA`, `Footer`, `[data-variant="hybrid"]`) with the static `home/Hero` swapped for a playable canvas runner. As of the "Vibe Runner V2" cinematic redesign this is split into a standalone canvas engine (`src/components/home/runner/runner-engine.ts`, exporting `VibeEngine` + `WORLDS`) and a thin React overlay (`src/components/home/runner/VibeRunner.tsx` — marketing hero, HUD, world card, dead overlay). Inside the canvas each "world" (a product, service, or the YC backing) is its own **universe**: a 3-stop sky gradient (mostly dark), an optional celestial body, multi-layer parallax silhouettes, a near-field motif, its own ink/accent/player colours, and a **warp transition** (hyperspeed streaks + letterbox + slammed world-name) between worlds, plus a comet mascot ("Volt"). Difficulty still mirrors the Chrome/Firefox dino algorithm. Canvas `ctx.font` cannot resolve `var(--font-geist-*)`, so `VibeRunner` resolves the concrete family names via `getComputedStyle` and passes them into `new VibeEngine(canvas, hooks, fonts)`. It is a preview, not yet the canonical `/` homepage; when it replaces `/`, swap `Hero` for `VibeRunner` in `HomeLaunchpad` and promote the DESIGN.md prose.
