...
**Output**: A design brief that can be handed off to the craft flow, or directly to Impeccable for freeform implementation. When visual direction probes are used, the images are supporting artifacts, not the primary output.
...
This is a required interaction, not optional guidance. Ask these questions in conversation, adapting based on answers. Don't dump them all at once; have a natural dialogue. Ask the user a concise question and wait for the answer.
...
Ask the user a concise question and wait for the answer. Ask for explicit confirmation of the brief before finishing.
...
Once confirmed, the brief is complete. The user can now hand it to Impeccable, or use it to guide any other implementation approach. (If the user wants the full discovery-then-build flow in one step, they should use the craft flow instead, which runs this command internally.)
